...
**
** Pour avancer de ligne en ligne avec un buffer de une dimmension :
** avancer de size_ligne en size_ligne.
** Pour avancer d un pixel dans une ligne il faut avancer de bite par pixel / 8.
** (un byte = 8 bites)
** Mettre un int dans un char* = on recupere l adresse de data(&).
** On le cast en int*. + *devant pour y acceder.
**
** On cast directement mlx_get_data_adr() du coup on le stock dans un int *data.
** On peux supprimer le cast du debut
** puisque c'est deja un int qu'on veux mettre dans un int.
** Vu que c'est un int* on avance de 4.
** Donc on supprime le '* img->bits_per_pixel / 8' car c'etait = 4.
*/

static int	ret_color(t_env *env, int red, int green, int blue)
{
	int		all;
	t_color	color;
...
void		draw_column(t_env *env)
{
	...
	while (y < env->res.y)
	{
		...
		{
			color = ret_color(env, env->c.red, env->c.green, env->c.blue);
...
** 1) c'est devant le plan de la caméra donc on ne voit pas pas derrière
...
void		draw_sprite(t_env *env, int i)
{
	...
	while (++x < env->spr[i].dend.x)
	{
			...
			&& env->spr[i].tform.y < env->zbuffer[x])
		{
			...
			{
				env->spr[i].tex.y =
					(((y * 256 - env->res.y * 128 + env->spr[i].h * 128)
					* env->tx[S].height) / env->spr[i].h) / 256;
